...
当前状态：v0.7，已接入 LLM 增强的 LangGraph 工作流分析、结构化报告、项目画像识别、
...
- 支持 LLM 增强的 workflow 分析。
- 支持可 mock 的 LLM 分析节点，测试不依赖真实模型。
- 默认仍为 `--no-llm` 确定性模式。
- 配置 API Key 后，可通过 `--with-llm` 生成更深入的结构化 `AnalysisReport`。
...
repoinsight workflow "分析这个项目是否适合开源发布" --path ./some-project --with-llm
...
python -m repoinsight.cli workflow "分析这个项目是否适合开源发布" --path . --with-llm
...
使用 `--with-llm` 时需要 `OPENAI_API_KEY`，LLM 只基于 workflow 已收集的
profile、plan 和 evidence 生成结构化 `AnalysisReport`。
...
- 配置 API Key 后，可以运行 `--with-llm` 的 LLM 增强 workflow。
- 不支持任意 shell 命令。
- 不支持安装依赖。
- 暂不支持修改源码文件。
...
下写入生成报告。
使用 `--with-llm` 时，LLM 不直接访问文件系统、不直接执行命令、不直接写文件；
它只能基于 workflow evidence 生成 `AnalysisReport`。写文件仍由 `report_node`
调用结构化报告写入工具完成，并且只写入 `reports/`。
...
- v0.7：加入 LLM 增强的 workflow analyzer。
- v1.0：开源稳定版本。
